...
namespace JellyfinMod.Data.Migrations
{
    ...
    public partial class PhaseTenMovieBacklog : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // History says why. A movie scheduled from a watch after the floor is left as it is. The Phase 3 rule stored the
            // time the plugin read a played flag that carried no date as the completion (RET4-R3): such a basis can lie after
            // the floor although nothing dates the watch, so a movie counts as fresh only when one of its observations is
            // played with Jellyfin's own last-played instant at or after the floor.
            const string floor =
            ...
            const string backlog =
                "e.\"EpisodeId\" IS NULL AND e.\"State\" = 'scheduled' AND (e.\"CompletionBasisAt\" IS NULL OR " +
                "e.\"CompletionBasisAt\" < " + floor + " OR NOT EXISTS (SELECT 1 FROM \"CompletionObservations\" o " +
                "WHERE o.\"TargetId\" = e.\"TargetId\" AND o.\"Played\" = 1 AND o.\"LastPlayedAt\" IS NOT NULL AND " +
                "o.\"LastPlayedAt\" >= " + floor + "))";
            migrationBuilder.Sql(
                ...
                "(decision 12); this one has no dated watch after that', " +
